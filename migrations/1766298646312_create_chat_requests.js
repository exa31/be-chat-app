...
exports.up = (pgm) => {
    ...
    // create enum type for status (include blocked)
    pgm.sql("CREATE TYPE chat_request_status AS ENUM ('pending','accepted','rejected','blocked')");
    ...
    pgm.createTable('chat_requests', {
        ...
        rejected_reason: {type: 'text', notNull: false},
        cooldown_until: {type: 'timestamp with time zone', notNull: false}
    });

    // prevent sending request to self
    pgm.sql("ALTER TABLE chat_requests ADD CONSTRAINT chat_requests_no_self CHECK (sender_id <> receiver_id)");

    // only one PENDING request between two users regardless of direction - allow historical rows (rejected/accepted/blocked)
    // use a partial unique index on the unordered pair when status = 'pending'
    pgm.sql("CREATE UNIQUE INDEX chat_requests_pair_unique_pending ON chat_requests (LEAST(sender_id::text, receiver_id::text), GREATEST(sender_id::text, receiver_id::text)) WHERE status = 'pending'");
